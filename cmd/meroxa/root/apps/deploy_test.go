...
func Test_validateFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "With --spec and with --env flags for go",
			specFlag: ir.SpecVersion_0_2_0,
			lang:     ir.GoLang,
			envFlag:  "my-env",
		},
		{
			name:     "With --spec and with --env for py",
			specFlag: ir.SpecVersion_0_2_0,
			lang:     ir.Python,
			envFlag:  "my-env",
		},
		{
			name:     "With --spec and with --env flags for ruby",
			specFlag: ir.SpecVersion_0_2_0,
			lang:     ir.Ruby,
			envFlag:  "my-env",
		},
		{
			name:     "With --spec and with --env flags for js",
			specFlag: ir.SpecVersion_0_2_0,
			lang:     ir.JavaScript,
			envFlag:  "my-env",
		},
		{
			name:    "Without --spec and with --env flags if language is not ruby, go, js or py",
			envFlag: "my-env",
			lang:    "",
			wantErr: fmt.Errorf(
