...
func (*List) Docs() builder.Docs {
	return builder.Docs{
		Short: "List all Conduit Auditlog",
		Long: `This command will list all the Auditlog defined on the platform.
`,
		Example: `meroxa Auditlog list`,
	}
}
...
func (d *List) Execute(ctx context.Context) error {
	...
	err = json.NewDecoder(response.Body).Decode(&Auditlog)
	if err != nil {
		return err
	}

	d.logger.Info(ctx, display.PrintList(Auditlog.Items, displayDetails))
	d.logger.JSON(ctx, Auditlog)
	output := fmt.Sprintf("\n ✨ To view your Auditlog, visit %s/auditlog", global.GetMeroxaTenantURL())
