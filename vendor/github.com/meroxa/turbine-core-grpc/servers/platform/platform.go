...
import (
	...
	"errors"
	"fmt"
	"github.com/meroxa/turbine-core-grpc"
	"log"

	"github.com/meroxa/meroxa-go/pkg/meroxa"
	"github.com/meroxa/turbine-core-grpc/hotfix"
	pb "github.com/meroxa/turbine-core-grpc/lib/go/github.com/meroxa/turbine/core"
	"github.com/meroxa/turbine-core-grpc/platform"
...
func (s *server) WriteCollectionToResource(ctx context.Context, request *pb.WriteCollectionRequest) (*emptypb.Empty, error) {
	...
	err := hotfix.Apply(connectorConfig,
		request.Resource.Type,
		request.Collection.Stream,
		request.TargetCollection) // todo: move this to API
	if err != nil {
		return Empty(), err
	}
	log.Printf("connectorConfig (hotfix): %+v", connectorConfig)
...
func (s *server) findOrCreateApp(ctx context.Context, language string, gitSHA string) (string, error) {
	...
	inputCreateApp := &meroxa.CreateApplicationInput{
		...
		Language: language,
		GitSha:   gitSHA,
		Pipeline: meroxa.EntityIdentifier{Name: null.StringFrom(s.config.Pipeline)},
	}
